...
def filter_corpus(input_dir, output_dir):
    ...
    print(
        f"Filtering {len(c_files)} .c files for {len(RELEVANT_PATTERNS)} relevance patterns..."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter decompiled .c files for relevance"
    )
    parser.add_argument("--input", required=True, help="Directory containing .c files")
    parser.add_argument(
        "--output", required=True, help="Directory for filtered .c files"
    )
